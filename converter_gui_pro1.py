...
def convert_file(input_file, delimiter, encoding_choice, output_dir, progress=None):
    ...
    if ext == ".csv":
        df = pd.read_csv(input_file, dtype=str, encoding=encoding, sep=None, engine="python")
    else:
    ...
    table = dbf.Table(output_file, structure, codepage="cp1251")
    table.open(dbf.READ_WRITE)

    # Записуємо рядки — вручну кодуємо в cp1251, щоб dbf не чіпав типи
    for _, row in df.iterrows():
        ...
        for val in row:
            ...
            try:
                ...
                val = val.encode("cp1251", errors="replace").decode("cp1251")
            safe_row.append(val)
...
# --- GUI ---
root = tk.Tk()
root.title("Конвертер Excel/CSV → DBF (Pro v3.7)")
root.geometry("520x380")

cfg = load_config()

label = tk.Label(root, text="Конвертер Excel/CSV → DBF (Pro v3.7)", font=("Arial", 14))
label.pack(pady=10)
